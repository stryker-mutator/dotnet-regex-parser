...
namespace Stryker.Regex.Parser.UnitTest.Nodes.GroupNodes
{
    ...
    public class CommentGroupNodeTest
    {
        ...
        public void SpanOnCommentGroupShouldStartAfter()
        {
            ...
            ConcatenationNode concat = new(new List<RegexNode> { a, b });

            // Act
            var (commentStart, commentLength) = comment.GetSpan();
            (int aStart, int aLength) = a.GetSpan();
            (int bStart, int bLength) = b.GetSpan();
            (int concatStart, int concatLength) = concat.GetSpan();
